...
22. **Íkonakerfi (úr handoff frá öðrum þræði, lota 7).** Stílhreint íkonakerfi: blár fleti + hvít lína. `src/data/icons.jsx` — `CatIcon` (9 SVG: almennur poki + 8 flokkaíkon) + `iconKeyForDept` mapping (deild → íkon). `ListView` sýnir nú **flokkaíkon þegar innkaupavara hefur enga alvöru ljósmynd** (í stað þess að sýna ekkert / gömlu `generic_images` sem voru hvort eð er ónotuð í framenda). CSS `.cat-icon`. **Pallettu-tiltekt (gert):** `--accent`/`--info` færð í navy `#15315e` (sameinar bláa tóninn — ein breyta, auðvelt að afturkalla). Nýr `--done: #2ed3a4` (grænn = AÐEINS „búið") á öll hök (itemRow, wk-check, shopmode, ing-check, shelf-check). Stig (`.lb-points`, `.home-ach b`) í gull. Íkon líka í innkaupa-tillögum. **ÓGERT:** sömu íkon í Vöruhillu/ShoppingMode. Óprófað í Cowork.

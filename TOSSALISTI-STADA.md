...
35. **SEO-lendingarsíður (organic vöxtur) (lota 7n).** Appið er SPA (rankar illa), svo gerðar **sjálfstæðar statískar HTML-síður** í `public/` sem Google indexar beint: `utilegulisti.html` („Útilegulisti 2026" — allur pakkinn sýnilegur sem texti) og `innkaupalisti.html` („Innkaupalisti fyrir vikuna"). Hvor með title/meta/OG/canonical, efni sem rankar, og CTA á appið með `?utm_source=seo&utm_campaign=...`. Plús `robots.txt` + `sitemap.xml`. Netlify þjónar statískum skrám á undan SPA-redirect, svo þær virka. **Eftir deploy:** skrá sitemap í Google Search Console. Bæta við fleiri síðum úr sniðmátunum (sumarbústaður, grillveisla, skólabyrjun). Tengja `innkaupalisti.is` lén. **Óútgefið.**
    - **Tölvupóstur til notenda:** ákveðið að nota Gmail-BCC í bili (21 notandi — persónulegt slær sjálfvirku við). Notendalisti sóttur + drög að endurvirkjunar-pósti gefin. Sjálfvirka kerfið bíður láns+Resend (sjá minnislista).

34. **„Bjóddu heimilisfólki"-hvati (solo→multiplayer retention) (lota 7m).** Gögn sýndu 71% notenda í SÓLÓ-listum (6/21 í deildum) — sterkasta retention-tækifærið. Gyllt spjald á heimaskjá birtist AÐEINS ef notandi á engan deildan lista (`store.hasSharedList()` — athugar hvort hann sé meðlimur annars lista EÐA eigi lista með meðlimum). Smellur → `inviteHousehold`: `createInvite` + Web Share API (`navigator.share` → OS-deilisheet) með `?invite=`-tengli, fellur á ShareModal. Hverfur sjálfkrafa þegar notandi deilir. Endurnýtir alla deilingarvélina. **Óútgefið.**

33. **Gestateljari (vefgreining) á Stjórnborði (lota 7k).** Vantaði sýn á alla gesti (ekki bara skráningar). **Migration `add_pageviews` KOMIN Í LOFTIÐ:** tafla `pageviews` (path, source, utm_campaign, referrer, visitor, created_at), RLS leyfir anon+authenticated INSERT (svo gestir fyrir innskráningu teljist), `admin_pageviews()` SECURITY DEFINER (admin-netfang) skilar views_24h/7d, einstökum gestum (visitor-id), signups_7d + breytingu, by_source, by_day. Nafnlaust: `visitorId()` = slembi-id í localStorage (engin vafrakaka — privacy-first). `attribution.js`: `visitorId`, `pageContext`. App: `store.logPageview(pageContext())` einu sinni á hleðslu (óháð innskráningu). AdminView: nýr „Umferð"-hluti. **ATH:** telur frá núna (engin afturvirk gögn); eigin heimsóknir admins teljast líka með. **Óútgefið.**

32. **Push-áminningar — villuleit & lagfæringar (lota 7i).** Áminningar bárust aldrei. Rakið í gegn:
    - **Netlify búið með bygginga-mínútur** → ný commit fóru ekki í loftið (hélt síðustu virku útgáfu). LAUSN: handvirkt `dist`-upphlað (drag-drop í Netlify Deploys, eða `npx netlify deploy --prod --dir=dist`) — notar engar bygginga-mínútur. Mínútur endurnýjast mánaðarlega.
    - **`push_subscriptions`-taflan var ÞEGAR til** (úr eldra skema) með dálkum `endpoint,p256dh,auth,platform` — EKKI `subscription`-jsonb sem kóðinn skrifar í; `create table if not exists` sleppti að búa hana til. Hver vistun klikkaði þögult → 0 áskriftir. LAUSN migrations: `fix_push_subscriptions_add_subscription_col` (bætti `subscription jsonb`+`user_agent`) og `push_subscriptions_relax_legacy_notnull` (gerði p256dh/auth/platform valkvæð). `savePushSubscription` hendir nú villu (ekki þögul). **Staðfest enda-í-enda: push barst kl. 20:45.** Cron+edge+VAPID allt í lagi.
    - **Bjöllu-vistun skráði ekki `reminder_enabled`** (3 aðskildar vistanir, gat bilað þögult). LAUSN: `setItemReminderFull` — eitt atómískt update (reminder_enabled+time+due) með villu ef 0 línur uppfærast (RLS-vörn); reminderSheet sýnir nú villu. **Óútgefið — bíður dist-upphlaðs.**
    - LEXÍA: alltaf athuga hvort tafla sé þegar til með annað skema áður en treyst er á `create table if not exists`.

31. **Lærdóms-flokkun (kerfið lærir) (lota 7g).** Notandi vildi að flokkun lærðist: leiðrétti „Ballerina kex" → snakk, „bananar" → matur, og næst þegar skannað er sé það rétt. **Migration `add_learned_categories`** KOMIN Í LOFTIÐ: tafla `learned_categories (user_id, name_norm, category)` per-notanda, RLS. `categories.js`: módúl-staðbundið `LEARNED` + `setLearnedCategories(map)`; `itemCategory(name)` ráðfærir sig nú við LEARNED **fyrst**, svo reglu-flokkun (`departmentFor`/DEPT_TO_CAT), svo „annad". store: `getLearnedCategories` / `learnCategory(name,cat)` (cloud+local). App: hleður lærða flokka við ræsingu → `setLearnedCategories`; `setItemCat(id,cat,name)` lærir nafnið + endurhleður lærða kortið áður en endurteiknað er. BudgetView: línuliða-flokkari sendir nú `it.name` með + lítil skýring „Kerfið man þennan flokk fyrir „X" næst". Virkar bæði afturvirkt (aðrar færslur með sama nafni og null-flokk endurflokkast við birtingu) og fram í tímann (nýjar skannanir). **Takmörkun:** lærdómur er á nákvæmt (normalíserað) vöruheiti — „Bananar lausir" lærist ekki af „bananar". Nóg fyrir endurteknar kvittanir úr sömu verslun. **Frontend óútgefinn.**

30. **Kostaðar auglýsingar á rofa — stýrt af admin (lota 7e).** Notandi vildi SLÖKKVA á kostuðu efni (Ölgerðin) en geyma kóðann til að kveikja aftur fyrir fund með Óla. Lausn: altæk stilling, ekki eyðing. **Migration `add_app_settings_ads_flag`** KOMIN Í LOFTIÐ: tafla `app_settings (key, value jsonb)`, RLS (allir lesa, aðeins admin-netfang skrifar), seed `ads_enabled=false`. store: `getAppSettings` / `setAppSetting`. App: hleður stillingar, `adsEnabled` (sjálfgefið false), `toggleAds`, þrætt í HomeView/ListView/AdminView. **Gataðir fletir (allir faldir þegar slökkt):** `AdBanner` (heimaskjár + listi), kostaðar drykkjar-tillögur (`sponsoredSuggest`), kostaða vörustrípan í deildarhausum (`CATEGORY_SPONSORS`), og „Kostað"-hillan í `ShelfView`. **Stjórnborð:** nýr rofi „Kostaðar auglýsingar" efst (aðeins admin = sigursveinsson@gmail.com) — kveikir/slekkur fyrir alla. ATH: aðrir notendur sjá breytinguna við næstu hleðslu/innskráningu (app_settings sótt við session-start). Fyrir Óla-fund: kveikja → endurhlaða. **Kóðinn er allur óbreyttur, bara falinn.** Frontend óútgefinn.

29. **Persónuvernd / samþykkis-skjár (lota 7d).** Fyrsta-skiptis samþykki sem (a) fær samþykki notenda og (b) kynnir „besta verð"-framtíðarsýnina. **Migration `add_profile_consent`** (consent_at, consent_version á profiles) KOMIN Í LOFTIÐ. store: `recordConsent(version)` + getMyProfile skilar nú consent_at/version. `ConsentNotice.jsx` birtist eftir ProfileSetup ef `!profile.consent_at` (líka fyrir núverandi notendur — afturvirkt samþykki). Orðalag (heiðarlegt — nafnleysi bundið við samanteknu gögnin, EKKI hráu kvittunina sem er persónuupplýsing): „Tossalisti vistar kvittanirnar þínar… í framtíðinni viljum við nota gögn í samanteknu og nafnlausu formi til að hjálpa þér að finna besta verðið… við seljum aldrei persónuupplýsingar… þú getur slökkt hvenær sem er." Útvíkkanlegur „nánari upplýsingar" hluti. **ATH:** GDPR-samþykki á að vera sértækt — raunveruleg framtíðar-notkun þarf sína eigin tilkynningu/grunn; fá lögfræði-yfirferð áður en treyst er formlega. **Frontend óútgefinn.**

28. **Sýnilegri deiling (lota 7c).** Deiling var bara grafin undir þriggja-punkta aðgerð á lista í fellivalmynd → fólk fann hana seint, fáir deildu (veirulykkjan veik). Bætti **gylltum ⤴ deila-hnappi í hausinn** (App.jsx) þegar listi er opinn (ekki í bókhaldi), kallar `openShare(list)`. Sýnilegur meðan unnið er í listanum. CSS `.share-btn` (cta-litur). Hugmynd síðar ef vill: líka lítið deila-tákn á hverja röð í ListsPanel. **Óprófað í Cowork.**

27. **Samhengisháð áminninga-bjalla (lota 7b).** Bjallan kveikti áður á áminningu en tíminn var falinn í stillingum verksins — og verklistar (type=task) höfðu engan tíma, svo áminning á slíku verki fór aldrei af stað. Lagað: bjalla (þegar slökkt) opnar nú lítinn `reminderSheet` í ListView sem spyr um réttu reitina eftir tegund: **vikuskema** → tími (dagur kemur af verkinu); **verklisti** → dagsetning + tími. Vistar svið + kveikir á áminningu í einni hreyfingu. Bjallan sýnir tímann þegar virk (🔔 16:00). Engin edge-function breyting þurfti (reglan var þegar: (weekday==í dag ‖ due_at==í dag) OG time til staðar). **Óprófað í Cowork.**
...
- **Hóp-/vinnustofu-eining (workspace) fyrir vinnustaði/húsfélög/félög.** Í dag er „hópur" = deildur listi (list_members). Vantar: ein hóp-eining með föstum meðlimalista + hlutverk (admin/meðlimur) sem á MARGA lista (svo húsfélag deili ekki hverjum lista fyrir sig). **ÁKVÖRÐUN (lota 7): geymt — annar markaður (B2B/teymi) en neytenda-heimili. Stefna: klára neytendavöruna fyrst, sanna retention; EF Tossalisti nær flugi, gera B2B-útgáfu sem sérvöru ofan á sama gagnalíkani.** V1-skissa ef/þegar: `workspaces` + `workspace_members` (admin/meðlimur) + valkvæður `workspace_id` á `lists` + svæða-skiptir í hausnum; halda list_members fyrir stök óformleg deiling; sleppa fínkornóttum réttindum fyrst (allir meðlimir sjá alla lista svæðisins).
...
- **Channel conflict — verðgagnsæi vs. samstarf við verslun/heildsala (lota 7j).** Áhætta: ef Tossalisti birtir þvert-á-verslanir verðsamanburð gæti það ýtt undir samkeppni → lægra verð → fælt heildsala/verslanir frá samstarfi. Aðgreining: (a) **persónulegt bókhald** (þín eigin eyðsla úr þínum kvittunum) ógnar engum; (b) **retail-media/kynning** (Ölgerðin borgar fyrir sýnileika, EKKI verð) samræmir hagsmuni heildsala við okkar — bandamaður; (c) **opinber verðsamanburður** = neytenda-talsmanns-hlutverk, stillir okkur á móti versluninni. Stefna: leiða með (b)+persónulegu (a); meðhöndla (c) sem sérstaka, meðvitaða seinni ákvörðun (flytur okkur hinum megin við borðið). Fordæmi á Íslandi: ASÍ-verðlagseftirlit + Neytendasamtök birta verðsamanburð. Óla-skilaboð: „kynning, ekki verð".
...
- **Tölvupóst-kerfi (re-engagement + skilaboð).** Senda notendum tölvupóst — t.d. vikulegan sunnudags-hvata „skipuleggðu vikuna", virkjunar-póst, eða stjórnenda-skilaboð. **Forsendur áður en byggt:** (1) staðfesta `tossalisti.is` lén (SPF/DKIM), (2) Resend (eða álíka) API-lykill sem edge-function secret. Útfærsla = sama mynstur og áminningakerfið: edge-fall `send-emails` + pg_cron, les notendur+stillingar, sendir í gegnum Resend. Þarf líka afskráningar-hlekk (CAN-SPAM/GDPR) + email-stillingu í notendaprófíl (opt-out). ÁKVÖRÐUN (lota 7l): geymt þar til lén+Resend eru klár; nota push í millitíðinni.
